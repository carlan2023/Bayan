...
## Milestone 6 — UX, accessibility & SEO pass ✅

Run after the white-label, variants and payments work landed, so it covers the pages those added. Measured with axe-core in Chromium across 17 pages (storefront and admin, signed in and out) at 1280px, and the four busiest at 390px: **about 45 violations before, 0 after.**

- [x] **Contrast.** White on the clay accent was 4.18:1 (buttons, sale badges, cart count, admin active nav, notification badge). New `--clay-fill` token: the accent darkened just enough for 4.5:1 (`#b06a4d` → `#aa6447`, barely visible). `theme.js` computes it for any owner palette (`readableFill`), and picks dark text over a light primary (`bestText`), so a pastel palette chosen in Settings can't make labels unreadable. The WhatsApp button (white on `#25d366`, 1.98:1) now has dark green text; the footer small print no longer fades to 3.4:1.
- [x] **Structure.** Every page has one `<h1>` (empty cart, empty checkout, the thank-you screen and 404 now do); heading levels no longer skip (admin panels `h3` → `h2`, a hidden results heading on the catalogue); the announcement bar is a labelled landmark; the sort select and each order's status select have names; action columns have (visually hidden) headers.
- [x] **Keyboard.** A "Skip to content" link; focus moves to the page on client-side navigation (previously it stayed on the clicked link, so screen readers announced nothing); one visible `:focus-visible` ring everywhere (three `outline: none` rules on inputs removed); reduced-motion respected.
- [x] **Announcements.** Per-page titles (`usePageTitle`) so navigation is announced and tabs are distinguishable; "Added to bag" is spoken; errors on the new forms are `role="alert"`; `<html lang>` follows the shop's locale.
- [x] **SEO.** The server writes each page's title, description, canonical URL, Open Graph/Twitter tags and schema.org JSON-LD into the shell (`backend/src/seo.js`). Products get `Product` data with `Offer`/`AggregateOffer` and live availability; the home page gets `Store` and `WebSite` with a search action. Private pages (admin, checkout, account, tracking…) and search results are `noindex`. Unknown URLs and missing products answer **404** (still rendering the SPA's not-found page) instead of 200. `robots.txt` and `sitemap.xml` (every product and department) are generated.
- [x] **Link previews.** A product shared on WhatsApp now previews with its name, description and photo rather than just the shop name, since `og:*` tags are in the HTML the preview bot fetches.

**Not verified:** a real screen reader (NVDA/VoiceOver) pass. The checks were axe plus scripted keyboard tests. Google's Rich Results test and a real WhatsApp preview need the deployed URL.
